...
<title>相册管理 - <?php echo ($gallary["title"]); ?></title>
...
<div class="page-header">
  <h1>相册 - <?php echo ($gallary["title"]); ?>&nbsp;<small>记录点滴生活</small></h1>
</div>
</div>
<div class="row">
  <div class="col-12" style="margin-bottom: 10px;">
    <a type="button" class="btn btn-lg btn-success" href=<?php echo U('Gallary/uploadPhotos', 'id='.$gallary['id']);?>>上传照片</a>
  </div>
</div>

<div class="row">
  <script type="text/javascript">photoList = [];</script>
  <?php if(is_array($photos)): $i = 0; $__LIST__ = $photos;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$photo): $mod = ($i % 2 );++$i;?><script type="text/javascript">photoList.push(<?php echo json_encode($photo); ?>);</script>
    <div id="photo-item<?php echo ($photo["id"]); ?>" class="img-container">
      <a class="img" href="javascript:void(0);" onclick="showPhotoInModal(<?php echo ($photo["id"]); ?>);">
        <div class="img-inner" >
          <img src="<?php echo ($photo["url"]); ?>" alt="<?php echo ($photo["title"]); ?>" />
        </div>
      </a>
      <div class="desc"><?php echo ($photo["title"]); ?></div>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <div class="img-modal-container">
          <span class="img-modal-span"><img class="img-modal" id="img-modal" src="<?php echo ($photo["url"]); ?>" /></span>
        </div>
        <div>
...
function updatePhoto() {
  ...
  xhr.open('post', "<?php echo U('updatePhotoHandler');?>");
...
function setCover() {
  var gallaryId = <?php echo ($gallary["id"]); ?>;
  var photoId = document.getElementById('inputId').value;
  var formData = new FormData();
  formData.append('cover_photo_id', photoId);
  formData.append('id', gallaryId);
  var xhr = new XMLHttpRequest();
  xhr.open('post', "<?php echo U('updateHandler');?>");
  $('.form-btn').attr('disabled','disabled');
  xhr.onload = function() {
    $('.form-btn').removeAttr('disabled');
    if (xhr.readyState==4 && xhr.status==200) {
...
function deletePhoto() {
  ...
  formData.append('id', photoId);
  var xhr = new XMLHttpRequest();
  xhr.open('post', "<?php echo U('deletePhotoHandler');?>");
  $('.form-btn').attr('disabled','disabled');
  xhr.onload = function() {
    $('.form-btn').removeAttr('disabled');
    if (xhr.readyState==4 && xhr.status==200) {
